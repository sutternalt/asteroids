...
public class TableQuad
{
	...
	private int nextIndexQuad(int i, int count) //quadratic probing; returns i+2^count mod length
	{
		int retVal = i + (int)Math.pow(2, count);
	...
	public Object put(Object key, Object element)
	{
		...
		{  // The key is not yet in this TableDouble.
			...
			{
				index = nextIndexQuad(index, count);
				count++;
			}
			keys[index] = key;
			data[index] = element;
